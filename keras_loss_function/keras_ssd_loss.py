...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        total_loss = (class_loss + tf.cast(self.alpha,tf.float32) * tf.cast(loc_loss,tf.float32)) / tf.math.maximum(tf.cast(1,tf.float32), tf.cast(n_positive,tf.float32)) # In case `n_positive == 0`
