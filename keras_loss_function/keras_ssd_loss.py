...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        classification_loss = tf.cast(self.log_loss(y_true[:,:,:-12], y_pred[:,:,:-12]), tf.float64) # Output shape: (batch_size, n_boxes)
        localization_loss = tf.cast(self.smooth_L1_loss(y_true[:,:,-12:-8], y_pred[:,:,-12:-8]), tf.float64) # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets.

        # Create masks for the positive and negative ground truth classes.
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        positives = tf.cast(tf.math.reduce_max(y_true[:,:,1:-12], axis=-1), tf.float64) # Tensor of shape (batch_size, n_boxes)
        ...
        neg_class_loss_all = classification_loss * tf.cast(negatives,tf.float64) # Tensor of shape (batch_size, n_boxes)
        ...
        total_loss = (tf.cast(class_loss,tf.float32) + tf.cast(self.alpha,tf.float32) * tf.cast(loc_loss,tf.float32)) / tf.math.maximum(tf.cast(1,tf.float32), tf.cast(n_positive,tf.float32)) # In case `n_positive == 0`
